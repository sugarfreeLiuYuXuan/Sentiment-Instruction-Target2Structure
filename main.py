...
def init_args():
    ...
    parser.add_argument("--do_pic", action='store_true', 
                        help="Whether to pic")
    ...
    output_dir = f"outputs_v2/{args.dataset}"
...
class T5FineTuner(pl.LightningModule):
    ...
    def train_dataloader(self):
        train_dataset = get_dataset(tokenizer=self.tokenizer, data_dir=args.dataset ,type_path="train", args=self.hparams)
    ...
    def val_dataloader(self):
        val_dataset = get_dataset(tokenizer=self.tokenizer, data_dir=args.dataset, type_path="dev", args=self.hparams)
...
def evaluate(data_loader, model, sents):
    ...
    for batch in tqdm(data_loader):
        ...
        outs = model.model.generate(input_ids=batch['source_ids'].to(device), 
                                    ...
                                    max_length=1024)  # num_beams=8, early_stopping=True)
...
def get_visual_dataset(tokenizer, args):
    E_I_list = ["EAEO", "EAIO", "IAEO", "IAIO"]
    dataset_list = []
    for ei in E_I_list:
        visual_dataset = ABSADataset(tokenizer=tokenizer,
                                     data_dir=args.dataset, 
                                    data_type='train',
                                    max_len=args.max_seq_length,
                                    E_I=ei,
                                    use_sent_flag=args.use_sent_flag,
                                    use_prompt_flag=args.use_prompt_flag)
        dataset_list.append(visual_dataset)
    return dataset_list

def get_visual_embeddings(args):
    print("Reload the model for picture")
    save_dir = args.output_dir
    device = torch.device(f'cuda:{args.n_gpu}')    # test GPU 5
    tokenizer = T5Tokenizer.from_pretrained(save_dir)
    tfm_model = T5ForConditionalGeneration.from_pretrained(save_dir, output_hidden_states=True)
    model = T5FineTuner(args, tfm_model, tokenizer)

    dataset_list = get_visual_dataset(tokenizer, args)
    all_embeddings_for_all_dataset = []
    for datset_item in dataset_list:
        all_embeddings = []

        for i in range(len(datset_item)):
            sample = datset_item[i]
            inputs = {'input_ids': sample['source_ids'].unsqueeze(0).to(device),
                      'attention_mask': sample['source_mask'].unsqueeze(0).to(device),
                      'decoder_input_ids': sample['target_ids'].unsqueeze(0).to(device),
                      'decoder_attention_mask': sample['target_mask'].unsqueeze(0).to(device)}
            model.eval()
            
            model.model.to(device)
            with torch.no_grad():
                #last_hidden_state = model(**inputs).encoder_last_hidden_state
                last_hidden_state = model(**inputs).decoder_hidden_states[-1]
                # avg_pool_embedding = torch.mean(last_hidden_state, dim=1)
                all_embeddings.append(last_hidden_state.squeeze().cpu().numpy())

        all_embeddings = np.array(all_embeddings)
        print(all_embeddings.shape)
        all_embeddings_for_all_dataset.append(all_embeddings)
    return all_embeddings_for_all_dataset

def draw_visual(T_SNE_embeddings):
    ...
    labels = ["EAEO", "EAIO", "IAEO", "IAIO"]  # 标签列表
    label_colors = ['#82B0D2', '#FA7F6F', '#FFBE7A', '#8ECFC9']  # 标签颜色列表
    for i in range(len(T_SNE_embeddings)):
        # 将数据降维为2维
        tsne = TSNE(n_components=3, random_state=0, learning_rate=300)
        all_embeddings = T_SNE_embeddings[i]
        all_embeddings_3d = tsne.fit_transform(all_embeddings.reshape(all_embeddings.shape[0], -1))
        ax.scatter(all_embeddings_3d[:, 0], all_embeddings_3d[:, 1], all_embeddings_3d[:, 2], s=5, color=label_colors[i], label=labels[i])
        # all_embeddings_2d = tsne.fit_transform(all_embeddings.reshape(all_embeddings.shape[0], -1))
        # plt.scatter(all_embeddings_2d[:, 0], all_embeddings_2d[:, 1], s=5, color=label_colors[i], label=labels[i])
    # 可视化结果
    plt.legend()
    plt.show()
    plt.savefig("2023_3_16_pic/pic_mean_v2_3D", dpi=600, bbox_inches='tight')
    

# initialization
args = init_args()
if args.do_pic:
    all_embeddings_for_all_dataset = get_visual_embeddings(args)
    draw_visual(all_embeddings_for_all_dataset)
    sys.exit(0)
